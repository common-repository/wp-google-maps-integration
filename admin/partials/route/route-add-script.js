(function($) {
	...
	$( document ).ready(function() {
		...
		var table = $('#markertable').DataTable( {
			'scrollX': true
		});

		var waypoints = [];
		var stopover = [];
		$('#markertable tbody').on('click', 'input', function () {
			waypoints = $('#waypoints').val().split( ',' );
			stopover = $('#stopover').val().split( ',' );
			var id = $( this ).val();
			var cbtype = $( this ).attr('cbtype');
			if ( cbtype === 'waypoint' ) {
				if ( this.checked ) {
					if ( waypoints.length > 8 ) {
						$( this ).attr( 'checked', false );
						alert( 'Oops, waypoints reach 8 limit!' );// Unchecks it
					} else {
						waypoints.push( id );
					}
				} else {
				...
				if ( this.checked ) {
					...
					for ( var i = 1; i < stopover.length; i++ ) {
						if ( id === stopover[ i ] ) {
							stopover.splice( i, 1 );
						}
					}
				}
				$('#stopover').val( stopover );
			}

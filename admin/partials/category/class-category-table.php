...
class TWGM_Category_Table extends WP_List_Table {
	    
    function __construct() {
        global $status, $page, $total_items;;
        parent::__construct( array(
            'singular'      => __( 'Category', 'twgm' ),
            'plural'        => __( 'Categories', 'twgm' ),
            'ajax'          => false
        ));
    }
	...
	function column_iconpath( $item ) {
        return sprintf( '<img src="' . esc_url( $item['iconpath'] ) . '" style="width:30px; height:30px;">' );
	}
    ...
    function get_columns() {   
		$columns = array(
            ...
            'name' => __( 'Name', 'twgm' ),
            'description' => __( 'Description', 'twgm' ),
            'iconpath' => __( 'Icon', 'fwrtgm' )
        );
		return $columns;
	}

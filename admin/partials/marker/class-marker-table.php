...
class TWGM_Marker_Table extends WP_List_Table {
	
    function __construct() {
        global $status, $page, $total_items;
        parent::__construct( array(
            'singular'  => __( 'Marker', 'twgm' ),
            'plural'    => __( 'Markers', 'twgm' ),
            'ajax'      => false
        ));
    }
    ...
    function column_name( $item ) {
        ...
        $admin_url = admin_url( sprintf( 'admin.php?page=' . $table_page . '&action=sdel&id=%u', absint( $item['id'] ) ) );
        $actions = array(
            'edit' => sprintf( '<a href="?page=' . $add_page . '&id=%u">%s</a>', absint( $item['id'] ), __('Edit', 'twgm' ) ), 
	...
	function column_lat( $item ) {    
        return esc_html( $item['lat'] );
	}

	function column_lon( $item ) {
        return esc_html( $item['lng'] );
	}
    ...
    function get_columns() {
		$columns = array(
            ...
            'name' 		=> __( 'Name', 'twgm' ),
            'address' 	=> __( 'Address', 'twgm' ),
            'lat' 		=> __( 'Latitude', 'twgm' ),
			'lon' 		=> __( 'Longitude', 'twgm' )
        );	
		return $columns;
	}
    ...
    function prepare_items() {        
        ...
        $per_page = $this->get_items_per_page( 'markers_per_page', 20 ); 
